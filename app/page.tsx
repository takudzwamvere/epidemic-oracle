...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50/20 to-slate-50">
      {/* Header */}
      <header className="border-b border-emerald-200/30 bg-white/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <Link href="/datasets">
              <button className="px-4 sm:px-6 py-2 text-sm sm:text-base text-slate-600 hover:text-emerald-500 font-medium transition-colors duration-200">
                Datasets
              </button>
            </Link>
            <Link href="/dashboard">
              <button className="px-4 sm:px-6 py-2 text-sm sm:text-base bg-emerald-400 text-white hover:bg-emerald-500 transition-colors duration-200 font-medium">
                View Dashboard
              </button>
            </Link>
          </div>
      ...
      <section className="relative px-4 sm:px-6 lg:px-8 pb-16 sm:pb-24 bg-gradient-to-br from-emerald-500 to-emerald-600">
        <div className="max-w-4xl mx-auto">
          <div className="p-12 sm:p-16 text-white">
            <div className="text-center mb-8">
              <div className="w-16 h-16 bg-white/20 backdrop-blur-sm flex items-center justify-center mx-auto mb-6">
                <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
              </div>
              <h3 className="text-3xl sm:text-4xl font-light mb-4">Stay Informed</h3>
              <p className="text-lg text-white/90 max-w-2xl mx-auto font-light">
                Get weekly outbreak summaries, dataset updates, and critical health alerts delivered to your inbox.
              </p>
            </div>
            ...
            <form onSubmit={handleSubscribe} className="max-w-md mx-auto">
              <div className="flex flex-col sm:flex-row gap-3">
                <input
                  ...
                  required
                  className="flex-1 px-6 py-4 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-white/30 transition-all"
                />
                <button
                  type="submit"
                  disabled={isSubscribing}
                  className="px-8 py-4 bg-white text-emerald-600 font-medium hover:bg-slate-50 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                >
                  {isSubscribing ? 'Subscribing...' : 'Subscribe'}
                </button>
              </div>
              {subscribeMessage && (
                <p className="mt-4 text-center text-white/90 bg-white/10 backdrop-blur-sm py-3 px-4">
                  {subscribeMessage}
                </p>
              )}
            </form>
