...
// Typhoid AI Service
const generateTyphoidAnalysis = async (predictions: any[], nationalPrediction: any) => {
  ...
  const outbreakSeverity = totalCases > 1500 ? 'severe' : totalCases > 600 ? 'moderate' : 'contained';
  const responseLevel = highRiskProvinces.length >= 3 ? 'EMERGENCY' : highRiskProvinces.length >= 1 ? 'HIGH_ALERT' : 'MONITORING';
  
  const analysis = {
    nationalOverview: `🍽️ **Typhoid Threat Assessment**: Zimbabwe is experiencing a ${outbreakSeverity} typhoid outbreak with ${totalCases.toLocaleString()} projected cases. ${highRiskProvinces.length} provinces are at high risk, indicating ${highRiskProvinces.length >= 3 ? 'widespread food and water contamination' : 'localized food safety issues'}. The data suggests ${nationalPrediction.average_risk === 'High' ? 'accelerating transmission' : 'stable but concerning'} patterns.`,
    
    transmissionAnalysis: `🔍 **Transmission Dynamics**: Analysis indicates ${highRiskProvinces.length > 0 ? 'multiple contaminated food sources' : 'focused transmission points'}. Food safety data shows ${new Date().getMonth() >= 9 && new Date().getMonth() <= 11 ? 'festive season conditions increasing food handling risks' : 'routine food safety challenges'}. Street food safety appears ${totalCases > 1000 ? 'severely compromised' : 'partially regulated'}.`,
    ...
    provincialAnalysis: predictions.map(pred => {
      ...
      if (pred.risk_level === 'High') {
        analysis = `🚨 **CRITICAL SITUATION**: ${pred.province} shows rapid foodborne spread with ${cases} projected cases (${pred.growth_rate}). Immediate deployment of: 1) Food safety inspection teams, 2) Emergency water quality testing, 3) Community antibiotic distribution. Street food vendors showing high contamination levels.`;
      } else if (pred.risk_level === 'Medium') {
        analysis = `⚠️ **ELEVATED RISK**: ${pred.province} at moderate risk with ${cases} cases. Recommend: 1) Enhanced food vendor licensing, 2) Public health education campaigns, 3) Water source chlorination. Watch for clustering in ${cases > 300 ? 'urban markets' : 'rural trading centers'}.`;
      } else {
        analysis = `✅ **CONTROLLED ZONE**: ${pred.province} maintaining low transmission with ${cases} cases. Focus on: 1) Sustaining food safety standards, 2) Routine water quality monitoring, 3) Community awareness. Ideal for preventive food safety programs.`;
      }
      return { province: pred.province, analysis, risk: pred.risk_level };
    }),
    
    recommendations: [
      `🎯 **IMMEDIATE ACTIONS**: ${responseLevel} response activated. Deploy food safety teams to ${highRiskProvinces.map(p => p.province).join(', ')} within 48 hours.`,
      `🍽️ **FOOD SAFETY**: Emergency inspection of 90% of food vendors in high-risk areas. Temporary closure of non-compliant establishments.`,
      `🏥 **HEALTHCARE RESPONSE**: Pre-position ${Math.ceil(totalCases * 0.5)} antibiotic courses and ${Math.ceil(totalCases * 0.2)} IV fluid sets at health centers.`,
      `💧 **WATER QUALITY**: Emergency chlorination of water sources in ${highRiskProvinces.length} provinces. Distribute ${Math.ceil(totalCases * 0.3)} water purification tablets.`,
      `🔍 **SURVEILLANCE & TESTING**: Establish ${Math.ceil(highRiskProvinces.length * 3)} food and water testing stations. Train ${Math.ceil(totalCases * 0.02)} community health workers in case detection.`,
      `🎓 **PUBLIC HEALTH EDUCATION**: ${totalCases > 800 ? 'Launch emergency food safety campaign' : 'Maintain routine food safety messaging'} across all media platforms.`
    ],
    
    predictiveInsights: [
      `📈 **Outbreak Trajectory**: Models project ${totalCases > 1500 ? 'continued growth over next 3-4 weeks' : 'plateau within 2-3 weeks'} without intervention.`,
      ...
      `💰 **Resource Optimization**: Focus 70% of food safety resources on ${highRiskProvinces.length} high-risk provinces for maximum impact.`
    ]
  };
...
const AIAnalysisContent = ({ aiAnalysis }: { aiAnalysis: any }) => {
  ...
  return (
    <div className="space-y-8">
      {/* National Overview */}
      <div className="bg-gradient-to-r from-orange-50 to-amber-50 border-l-4 border-orange-500 p-6 rounded-none shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          <Utensils className="w-6 h-6 text-orange-600" />
          <h3 className="text-lg font-bold text-orange-900">National Outbreak Intelligence</h3>
        </div>
      ...
      <div className="bg-gradient-to-r from-lime-50 to-green-50 border-l-4 border-lime-500 p-6 rounded-none shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          <Shield className="w-6 h-6 text-lime-600" />
          <h3 className="text-lg font-bold text-lime-900">Strategic Response Framework</h3>
        </div>
...
const RecommendationItem = ({ text, index }: { text: string; index: number }) => {
  ...
  return (
    <div className="flex items-start gap-4 p-4 bg-white rounded-none border border-lime-200 hover:shadow-md transition-all duration-200">
      <span className="flex-shrink-0 w-6 h-6 bg-lime-100 text-lime-600 rounded-full flex items-center justify-center text-sm font-bold mt-1">
        {index + 1}
      </span>
...
const TyphoidPredictionPage = () => {
  const params = useParams();
  const disease = 'typhoid';
  ...
  const loadUploadedFiles = async () => {
    try {
      ...
      // Filter files to only show those with "typhoid" in the name (case insensitive)
      const typhoidFiles = files
        ?.filter(file => {
          const fileName = file.name.toLowerCase();
          return fileName.includes('typhoid') && fileName.endsWith('.csv');
        })
        .map(file => ({
          name: file.name,
          selected: false
  ...
  const parseCSVData = (csvText: string): any[] => {
    ...
    return lines.slice(1).map(line => {
      ...
      headers.forEach((header, index) => {
        ...
        const numericFields = ['year', 'month', 'confirmed_cases', 'recoveries', 'deaths', 'hospitalizations', 'water_contamination'];
  ...
  const generateProvincePredictions = (allData: any[]): any[] => {
    ...
    const predictions = Object.entries(provinceData).map(([province, totals]) => {
      ...
      let risk_level = 'Low';
      if (predicted_cases > 400) risk_level = 'High';
      else if (predicted_cases > 150) risk_level = 'Medium';
  ...
  const handleAIAnalysis = async () => {
    ...
    try {
      const analysis = await generateTyphoidAnalysis(predictions, nationalPrediction);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-none p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 capitalize">Typhoid Outbreak Predictions</h1>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <div className="flex items-center justify-center w-12 h-12 bg-orange-50 rounded-none">
      ...
      <div className="bg-white border border-slate-200 rounded-none p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Select 2025 Typhoid Datasets</h3>
            <p className="text-slate-600 text-sm">Choose which uploaded typhoid datasets to use for predictions</p>
          </div>
        ...
        {/* Auto-Alerts Toggle */}
        <div className="flex items-center gap-3 p-4 bg-orange-50 rounded-none border border-orange-200 mb-6">
        ...
        {uploadedFiles.length > 0 ? (
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-none border border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <span className="text-slate-600 text-sm">
                  {selectedFiles.length} of {uploadedFiles.length} typhoid files selected
                </span>
              </div>
          ...
          <div className="text-center py-8 text-slate-600 bg-slate-50 rounded-none border border-slate-200">
            ...
            <p className="font-medium">No typhoid datasets found</p>
            <p className="text-sm mt-1">Upload CSV files with &apos;typhoid&apos; in the filename to get started</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-none max-w-6xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-white p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold">Typhoid Outbreak Intelligence Analysis</h2>
            ...
            <div className="bg-slate-50 px-6 py-4 border-t">
              <div className="flex justify-between items-center text-sm text-slate-600">
                <div className="flex items-center gap-2">
                  <Brain className="w-4 h-4" />
                  <span>Epidemiological AI Analysis • Typhoid Outbreak Response System</span>
                </div>
...
export default TyphoidPredictionPage;