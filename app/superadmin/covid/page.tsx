...
// COVID-19 AI Service
const generateCovidAnalysis = async (predictions: any[], nationalPrediction: any) => {
  ...
  const outbreakSeverity = totalCases > 5000 ? 'severe' : totalCases > 2000 ? 'moderate' : 'contained';
  const responseLevel = highRiskProvinces.length >= 3 ? 'EMERGENCY' : highRiskProvinces.length >= 1 ? 'HIGH_ALERT' : 'MONITORING';
  
  const analysis = {
    nationalOverview: `🦠 **COVID-19 Threat Assessment**: Zimbabwe is experiencing a ${outbreakSeverity} COVID-19 outbreak with ${totalCases.toLocaleString()} projected cases. ${highRiskProvinces.length} provinces are at high risk, indicating ${highRiskProvinces.length >= 3 ? 'widespread community transmission' : 'localized clusters'}. The data suggests ${nationalPrediction.average_risk === 'High' ? 'accelerating transmission' : 'stable but concerning'} patterns.`,
    
    transmissionAnalysis: `🏙️ **Transmission Dynamics**: Analysis indicates ${highRiskProvinces.length > 0 ? 'multiple active transmission clusters' : 'focused transmission points'}. Variant analysis shows ${new Date().getMonth() >= 5 && new Date().getMonth() <= 8 ? 'winter season conditions favoring indoor transmission' : 'moderate transmission conditions'}. R-value appears ${totalCases > 4000 ? 'above 1.5 indicating rapid spread' : 'around 1.0 indicating controlled spread'}.`,
    ...
    provincialAnalysis: predictions.map(pred => {
      ...
      if (pred.risk_level === 'High') {
        analysis = `🚨 **CRITICAL SITUATION**: ${pred.province} shows rapid community spread with ${cases} projected cases (${pred.growth_rate}). Immediate deployment of: 1) Testing and contact tracing teams, 2) Emergency vaccination drives, 3) Hospital capacity expansion. Urban centers showing super-spreader potential.`;
      } else if (pred.risk_level === 'Medium') {
        analysis = `⚠️ **ELEVATED RISK**: ${pred.province} at moderate risk with ${cases} cases. Recommend: 1) Enhanced testing in hotspots, 2) Booster vaccination campaigns, 3) Public health messaging. Watch for workplace and school clusters.`;
      } else {
        analysis = `✅ **CONTROLLED ZONE**: ${pred.province} maintaining low transmission with ${cases} cases. Focus on: 1) Sustaining vaccination coverage >70%, 2) Sentinel surveillance, 3) Traveler screening. Ideal for maintaining economic activities.`;
      }
      return { province: pred.province, analysis, risk: pred.risk_level };
    }),
    
    recommendations: [
      `🎯 **IMMEDIATE ACTIONS**: ${responseLevel} response activated. Deploy rapid testing teams to ${highRiskProvinces.map(p => p.province).join(', ')} within 48 hours.`,
      `💉 **VACCINATION STRATEGY**: Scale up booster campaigns to cover 60% of vulnerable populations in high-risk provinces. Mobile vaccination units for remote areas.`,
      `🏥 **HEALTHCARE CAPACITY**: Prepare ${Math.ceil(totalCases * 0.05)} additional hospital beds and ${Math.ceil(totalCases * 0.1)} isolation facilities. Stockpile oxygen and ventilators.`,
      `🔍 **TESTING & TRACING**: Establish ${Math.ceil(highRiskProvinces.length * 3)} pop-up testing sites. Train ${Math.ceil(totalCases * 0.02)} contact tracers for digital and manual tracing.`,
      `📱 **DIGITAL SURVEILLANCE**: Implement symptom monitoring apps in ${mediumRiskProvinces.length + highRiskProvinces.length} provinces. Real-time variant sequencing.`,
      `🎓 **PUBLIC HEALTH MEASURES**: ${totalCases > 3000 ? 'Consider targeted restrictions in high-density areas' : 'Maintain current public health guidelines'}`
    ],
    
    predictiveInsights: [
      `📈 **Outbreak Trajectory**: Models project ${totalCases > 5000 ? 'peak in 3-4 weeks without intervention' : 'plateau within 2-3 weeks'} with current measures.`,
      `🎯 **Cluster Identification**: ${highRiskProvinces.slice(0, 2).map(p => p.province).join(' and ')} showing characteristics of variant-driven waves.`,
      `🕒 **Critical Window**: Optimal intervention period is next 10 days to prevent ${Math.ceil(totalCases * 1.4).toLocaleString()} potential cases.`,
      `💰 **Resource Allocation**: Focus 65% of resources on ${highRiskProvinces.length} high-risk provinces for maximum impact.`
    ]
  };
...
const AIAnalysisContent = ({ aiAnalysis }: { aiAnalysis: any }) => {
  ...
  return (
    <div className="space-y-8">
      {/* National Overview */}
      <div className="bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 p-6 rounded-none shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          <Microscope className="w-6 h-6 text-red-600" />
          <h3 className="text-lg font-bold text-red-900">National Outbreak Intelligence</h3>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border-l-4 border-blue-500 p-6 rounded-none shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          <Shield className="w-6 h-6 text-blue-600" />
          <h3 className="text-lg font-bold text-blue-900">Strategic Response Framework</h3>
        </div>
...
const RecommendationItem = ({ text, index }: { text: string; index: number }) => {
  ...
  return (
    <div className="flex items-start gap-4 p-4 bg-white rounded-none border border-blue-200 hover:shadow-md transition-all duration-200">
      <span className="flex-shrink-0 w-6 h-6 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-sm font-bold mt-1">
        {index + 1}
      </span>
...
const CovidPredictionPage = () => {
  ...
  const loadUploadedFiles = async () => {
    try {
      ...
      // Filter files to only show those with "covid" in the name (case insensitive)
      const covidFiles = files
        ?.filter(file => {
          const fileName = file.name.toLowerCase();
          return (fileName.includes('covid') || fileName.includes('coronavirus')) && fileName.endsWith('.csv');
        })
        .map(file => ({
          name: file.name,
          selected: false
  ...
  const parseCSVData = (csvText: string): any[] => {
    ...
    return lines.slice(1).map(line => {
      ...
      headers.forEach((header, index) => {
        ...
        const numericFields = ['year', 'month', 'confirmed_cases', 'recoveries', 'deaths', 'hospitalizations', 'icu_cases', 'ventilator_cases'];
  ...
  const generateProvincePredictions = (allData: any[]): any[] => {
    ...
    const predictions = Object.entries(provinceData).map(([province, totals]) => {
      ...
      let risk_level = 'Low';
      if (predicted_cases > 800) risk_level = 'High';
      else if (predicted_cases > 300) risk_level = 'Medium';
  ...
  const handleAIAnalysis = async () => {
    ...
    try {
      const analysis = await generateCovidAnalysis(predictions, nationalPrediction);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-none p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 capitalize">COVID-19 Outbreak Predictions</h1>
      ...
      <div className="bg-white border border-slate-200 rounded-none p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Select 2025 COVID-19 Datasets</h3>
            <p className="text-slate-600 text-sm">Choose which uploaded COVID-19 datasets to use for predictions</p>
          </div>
        ...
        {/* Auto-Alerts Toggle */}
        <div className="flex items-center gap-3 p-4 bg-red-50 rounded-none border border-red-200 mb-6">
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-none border border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <span className="text-slate-600 text-sm">
                  {selectedFiles.length} of {uploadedFiles.length} COVID-19 files selected
                </span>
              </div>
          ...
          <div className="bg-white rounded-none max-w-6xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-white p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold">COVID-19 Outbreak Intelligence Analysis</h2>
            ...
            <div className="bg-slate-50 px-6 py-4 border-t">
              <div className="flex justify-between items-center text-sm text-slate-600">
                <div className="flex items-center gap-2">
                  <Brain className="w-4 h-4" />
                  <span>Epidemiological AI Analysis • COVID-19 Outbreak Response System</span>
                </div>
...
export default CovidPredictionPage;